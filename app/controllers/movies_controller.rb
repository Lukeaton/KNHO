...
# or use an exisiting one
  ...
  if local_director.present?
    ...
    if tmdb_response_director["profile_path"]
...
##Let's create the movie entry now
  ...
  movie.image = "#{BASE_IMAGE_URL}w185" + tmdb_response_movie["poster_path"]
  ...
  redirect_to movie
    ...
    private
    def movie_params
      params.require(:movie).permit(:title, :year, :plot, :genre, :rating, :review, :image, :overview)
    end
end
